...
export const UserGrowthChart = ({ analytics, range }: { analytics: AnalyticsData; range: TimeRange }) => {
    const stats = [
        { label: "24h", value: analytics.users.newLast24Hours ?? 0 },
        { label: "7d", value: analytics.users.newLast7Days ?? 0 },
        { label: "30d", value: analytics.users.newLast30Days ?? 0 },
    ];
    ...
    return (
        <div className="p-5 rounded-3xl bg-white border border-slate-100 shadow-sm h-full">
            <h3 className="text-lg font-bold text-slate-800">User Acquisition</h3>
            <p className="text-sm text-slate-400 font-medium mb-4">New users in selected period</p>
